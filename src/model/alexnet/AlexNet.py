...
class AlexNet(Model):
    def __init__(self, name = "AlexNet"):
        ...
        self.conv1 = layer.ConvolutionLayer(input = self.input, filter_shape = self.conv1_filter_shape, stride = self.conv1_stride, padding = self.conv1_padding)
        ...
        # 第二层开始分开了.
        self.pool1_columns = T.split(x = self.pool1.output, splits_size = [self.conv1_filter_shape[0] / 2] * 2, n_splits = 2, axis = 1)
        ...
        def get_conv3_conv4_conv5_column(idx):
            conv3_column = layer.ConvolutionLayer(  input = self.pool2_stacked, 
                                                    filter_shape = self.conv3_filter_shape, 
                                                    stride = self.conv3_conv4_conv5_stride, 
                                                    padding = self.conv3_conv4_conv5_padding, 
                                                    name = 'conv3_%d'%(idx)
                                                    )
            conv4_column = layer.ConvolutionLayer(input = conv3_column , 
                                                   filter_shape = self.conv4_filter_shape, 
                                                   stride = self.conv3_conv4_conv5_stride, 
                                                   padding = self.conv3_conv4_conv5_padding, 
                                                   name = 'conv4_%d'%(idx)
                                                   )
                                                   
            conv5_column = layer.ConvolutionLayer(input = conv4_column , 
                                                  filter_shape = self.conv5_filter_shape, 
                                                  stride = self.conv3_conv4_conv5_stride, 
                                                  padding = self.conv3_conv4_conv5_padding, 
                                                  name = 'conv5_%d'%(idx)
                                                  )
        ...
        self.output_layer = layer.SoftmaxOutputLayerWithLoss(input = self.fc2_output, n_in = 2*self.fc_hidden_units, n_out = self.output_units, label = self.label, name = 'SoftmaxOutputLayerWithLoss')
