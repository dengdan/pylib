...
def ls(path = '.', suffix = None):
    ...
    filtered = []
    for f in files:
        if utils.str_util.ends_with(f, suffix, ignore_case = True):
            filtered.append(f)
...
unit_dict = {"K" : SIZE_UNIT_K, "M" :  SIZE_UNIT_M, "G" : SIZE_UNIT_G}
def get_file_size(path:str, unit:str = 'K')->float:
    """获取文件大小

    Args:
        path (str): 文件路径
        unit (str, optional): _description_. Defaults to 'K'.

    Returns:
        float: 文件大小
    """
    unit = unit_dict[unit.upper()[0]]
...
def move(src, dest):
    ...
    if exists(dest):
        rm(dest)
    make_parent_dir(dest)
    shutil.move(get_absolute_path(src), get_absolute_path(dest))
